...
* Archivo: generador-llaveros-3d/assets/app/exportadores.js (`PRINTER_OVERRIDES`, `zSpansAreStratified`, `patchProjectSettings`) e index.html (regenerado)
* Cambio: ronda 4 del perfil, "que nada vuelva a tocar la meseta". Tres claves nuevas: `z_hop_types` `Auto Lift`→`Normal Lift` y `retraction_minimum_travel` 1→0.2 (ambas de IMPRESORA, en un `PRINTER_OVERRIDES` aparte porque `different_settings_to_system` las declara en el ÚLTIMO hueco del array, el del preset de impresora — en el hueco 0 el cargador las revertiría, el mismo fallo que ese bloque ya evitaba para las de proceso), y `enable_prime_tower: 0` condicionado a que los grupos de color no se solapen en Z (`zSpansAreStratified` mide los tramos Z reales de cada soup: el llavero apila base y letras, el arcoíris comparte capas y conserva la torre).
* Motivo: el usuario mostró el "mia" impreso: blanco perfecto, meseta rosa con rayones. Se laminó el 3MF real de producción con la CLI y se contaron los viajes de las capas blancas contra una máscara de la meseta: 264 viajes / 176 mm iban a la altura de capa —0.2 mm sobre el rosa recién planchado— con uno solo de 152.1 mm cruzando la pieza entera. La ronda 3 sí se aplica (`reduce_infill_retraction_mode = Disabled` en el volcado del gcode) pero arregla la retracción, no la elevación: con `Auto Lift` el salto de 0.4 no se ejecuta. La torre de purga, además, se imprimía en las 19 capas para un solo cambio de filamento; se comprobó que la purga real es IDÉNTICA con y sin ella (45.8 mm de filamento, la A1 la hace en su estación fuera de cama), así que quitarla no contamina el blanco.
* Relación: cuarta ronda del perfil anti-defectos; corrige el supuesto equivocado del comentario de la ronda 3. NO se tocó el planchado: se midió que deposita 0.03 mm (2.1× lo que usa Bambu en su propio `support_ironing_*`), pero son 30 µm contra una holgura de 200 µm — contribuye sin ser la causa, y bajarlo revertiría la ronda 2 anti-picaduras sin necesidad. Descartadas con números la banda angosta (granulometría morfológica: sólo 1.2 % del área rosa en rasgos <0.84 mm, no el 41 % que sugería una transformada de distancia sesgada) y el pillowing (el relleno queda a 1.2 mm de la cara con 6 capas macizas encima).
* Resultado: ✅ 103 de 103 pruebas; laminado del 3MF que produce el build nuevo: viajes sin salto sobre la meseta 264→156 (176→86 mm), peor viaje suelto 152.1→5.0 mm, recorrido total sin salto 533→164 mm, tiempo 983→889 s, filamento 1.90→1.21 g con el desperdicio de 0.83→0.14 g (era el 77 % del material). El caso arcoíris conserva la torre (8 cambios) como debe. Informe en audits/2026-08-13/acabado-rosa-llavero/REPORTE.md; versión 74eb85cb
## [2026-08-13]

* Archivo: generador-llaveros-3d/assets/app/exportadores.js (`colourChangeLayer`), creador.js (`updatePrintInfo`, `downloadBaseName` y los tres botones de descarga), tests/generator-colour-change-layer.test.mjs (nuevo) e index.html (regenerado)
* Cambio: (1) el aviso del modo "cambio de rollo" ya no calcula la capa con `floor` sino con la función pura `colourChangeLayer`, que redondea HACIA ARRIBA como hace el laminador, anuncia la altura REAL del cambio y la primera capa del color de arriba, y añade una advertencia cuando el grosor no es múltiplo de la altura de capa. (2) con un solo nombre en la placa, las descargas salen nombradas con ese nombre: `Maria_Jose.3mf`, `Maria_Jose.stl`, `Maria_Jose-por-colores.zip`; con varios se mantiene el genérico, y el lápiz conserva el sufijo `-lapiz` para que el mismo nombre en los dos productos no colisione.
* Motivo: (1) el laminador sólo corta en frontera de capa. Verificado laminando con la CLI: con capas de 0.20 mm una base de 2.5 termina el color de abajo en z=2.60 (capa 13), no en 2.40 — pero el aviso decía capa 13 como primera del color de arriba. El cliente sin AMS cambiaba el rollo una capa antes y le quedaba una capa entera de 0.2 mm del color equivocado sobre la base; encaja con los "puntitos blancos" reportados en el "mia". El deslizador de grosor va en pasos de 0.1 contra capas de 0.2, así que la mitad de los valores caían fuera de la rejilla. (2) petición del usuario: descargando de uno en uno, el archivo debe llegar con el nombre puesto en vez de seis "llaveros (3).3mf" en la carpeta de descargas.
* Relación: el hallazgo 4 del informe audits/2026-08-13/acabado-rosa-llavero/REPORTE.md, que quedó fuera de la ronda 4 del perfil por ser un problema distinto. `colourChangeLayer` vive en exportadores.js —y no en creador.js— porque creador.js es un IIFE sin exports y no se puede cubrir con pruebas; ahí sí queda expuesta y probada.
* Resultado: ✅ 108 de 108 pruebas (5 nuevas: rejilla exacta, desalineado que sube de capa, barrido de los 35 valores del deslizador contra las tres alturas de capa, divisores exactos y entradas inválidas). Verificado en preview con almacenamiento limpio: grosor 2.40 → "cambio a 2.40 mm, capa 13"; grosor 2.50 → "cambio a 2.60 mm, capa 14" con la advertencia, ambos idénticos a lo que produce el laminador; descargas con un nombre → Maria_Jose.{3mf,stl} y Maria_Jose-por-colores.zip, con dos nombres → llaveros.3mf (audits/2026-08-13/acabado-rosa-llavero/aviso-pausa-desalineado.png)
## [2026-08-13]

* Archivo: generador-llaveros-3d/assets/fuentes/{leckerli-one.ttf, leckerli-one-abc.woff2, fuentes.json}, plantilla.py, assets/app/creador.js, tests/static-audit.test.mjs e index.html (regenerado)
* Cambio: (1) Leckerli One entra como tipografía incluida número 16, junto a las script (tras Caveat), con su miniatura `Abc` en woff2 de 0.9 KB generada con `python -m fontTools.subset --text="Abc" --flavor=woff2`; el copy pasa de "quince" a "dieciséis tipografías". (2) El hash del versionado deja de depender del checkout: `plantilla.py` y la prueba del candado normalizan CRLF→LF antes de hashear. (3) `fuentes.json` entra en el hash y `creador.js` pide catálogo, TTF y miniaturas con el `?v=` de la app, leído del `src` de su propio script; las dos precargas del HTML se versionan igual para que no haya doble descarga.
* Motivo: (1) el usuario pidió dejar la fuente cargada de forma permanente en el sitio en vez de subirla en cada sesión. (2) el candado de versión estaba ROTO en main: `core.autocrlf=true` sin `.gitattributes` hace que el checkout de Windows deje CRLF y el del CI en Linux LF, así que el mismo commit producía un `?v=` distinto en cada máquina. El index commiteado decía `5b0b5bb7` y el CI calculaba `f3e2ef84`: la prueba 101 fallaba y `main` llevaba en rojo desde el merge del PR #2. (3) sin versionar `fuentes.json`, un navegador con caché vieja se queda con el catálogo anterior y la tipografía nueva no le aparece — la misma clase de rezago que motivó el versionado por hash.
* Relación: cierra el agujero que dejaba pasar 25c1edd (versionado por hash) y fbe717d (prueba de sincronía). NO se incluyó la fuente Walter que también pidió el usuario: declara `fsType = 1` (Restricted License embedding), es de Jenny Barck 2001 y no trae texto de licencia, así que redistribuirla en un sitio y un repo públicos es justo lo que ese bit prohíbe. Leckerli One sí: SIL OFL 1.1, `fsType = 0`, igual que las otras quince.
* Resultado: ✅ 108 de 108 pruebas, y el hash ya coincide calculado sobre CRLF y sobre LF (`c37fe3ad` en ambos), que es lo que desbloquea el CI. Verificado en preview: 16 tarjetas con Leckerli One en la posición 8, las 18 peticiones a assets/fuentes/ versionadas y ninguna duplicada, y "BETTY" en modo lápiz da 74×28 mm y 15.4 g sin subir ningún archivo — idéntico a lo que producía la fuente subida a mano (audits/2026-08-13/acabado-rosa-llavero/leckerli-incluida.png)
